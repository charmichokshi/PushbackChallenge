...
def estimate_pushback(now, airport_submission_format, etd) -> pd.Series:
...
def main(airport: str = None):
    ...
    for airport in airports:
        ...
        predictions = []
        for now in tqdm(pd.to_datetime(airport_submission_format.timestamp.unique())):
            predictions.append(estimate_pushback(now, airport_submission_format, etd))
    ...
    for airport in airports:
        airport_predictions_path = Path(f"validation_predictions_{airport}.csv.bz2")
        predictions.append(
            pd.read_csv(airport_predictions_path, parse_dates=["timestamp"])
        )

    predictions = pd.concat(predictions, ignore_index=True)
    predictions["minutes_until_pushback"] = predictions.minutes_until_pushback.astype(
        int
    )
